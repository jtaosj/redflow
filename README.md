...
- AI生成小红书风格图文大纲
- 支持多页面内容生成（1-12张可选）
- **11种风格选择**：小红书爆款、海报2K、INS极简、科技未来、自然清新、多巴胺、莫兰迪、黑金、极简白、赛博朋克、复古怀旧
- **风格一致性保证**：同一帖子内所有页面使用相同的用户选择风格
- 批量生成海报图片
- 编辑大纲和配图建议
- 一键下载完整内容（文字+图片）
- 生图完成后自动弹出完成提示
...
- 生成风格化产品图片
- 支持多种风格选择
- AI智能一键配置，自动推荐最佳风格和文案风格

### 历史记录
- 自动保存创作历史
- 查看历史作品详情
- 本地存储（最多20条，自动压缩图片节省空间）
- 优化保存逻辑，确保数据完整性
- 智能存储管理，存储空间不足时自动清理旧记录
...
### Google GenAI API
- **获取地址**: https://aistudio.google.com/app/apikey
- **图片生成模型**: `gemini-3-pro-image-preview`（默认，可在设置中自定义）
- **文本分析模型**: `gemini-2.5-flash`（默认，可在设置中自定义）
- **支持自定义端点和模型**
