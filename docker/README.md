...
```bash
# 构建镜像（带版本标签）
docker build -f Dockerfile.nginx --build-arg VERSION=2.2.0 -t redflow-nginx:2.2 .

# 运行容器
docker run -d -p 8080:80 --name redflow-nginx redflow-nginx:2.2
...
```bash
# 构建镜像（带版本标签）
docker build -f Dockerfile.node --build-arg VERSION=2.2.0 -t redflow-node:2.2 .

# 运行容器
docker run -d -p 3000:3000 --name redflow-node redflow-node:2.2
...
- **当前版本**: v2.2.0
- **镜像标签**: `redflow-nginx:2.2` 和 `redflow-node:2.2`
